...
class ObservedItem(models.Model):
    ...
    def send_notice(self, extra_context=None, headers=None):
        if extra_context is None:
            extra_context = {}
        extra_context.update({"observed": self.observed_object})
        send([self.user], self.notice_type.label, extra_context, headers)
...
def send_observation_notices_for(observed, signal="post_save", extra_context=None, headers=None):
    """
    ...
    for observed_item in observed_items:
        observed_item.send_notice(extra_context, headers)
    return observed_items
